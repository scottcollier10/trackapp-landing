...
import {
  buildPolylineFromGeoJSON,
  pointForPos as geoPointForPos,
  nearestPointOnPolyline,
  type Point,
} from "@/lib/geo-utils";
import cotaGeoJSON from "@/public/tracks/cota.json";
...
function TrackMap({
  ...
  useEffect(() => {
    try {
      const polyline = buildPolylineFromGeoJSON(cotaGeoJSON);
      setTrackPolyline(polyline);
    } catch (error) {
      console.error("Failed to build track polyline:", error);
    }
  }, []);

  const handleClick = (e: MouseEvent<SVGSVGElement>) => {
    const svg = svgRef.current;
    if (!svg || trackPolyline.length === 0) return;
    ...
    const { pos } = nearestPointOnPolyline({ x, y }, trackPolyline);
    onAddAtPos(pos);
  };

  // Helper: map 0–1 pos back to (x,y) for pins
  const pointForPos = (pos: number): Point => {
    if (trackPolyline.length === 0) {
      // Fallback to oval if polyline not loaded
      const cx = 200;
      const cy = 120;
      const rx = 110;
      const ry = 70;
  ...
  return (
    <div className="relative">
      ...
      {/* Track outline (COTA circuit) */}
      {trackPolyline.length > 0 ? (
        <>
          {/* Dark background stroke */}
          <path
            d={`M ${trackPolyline.map((p) => `${p.x},${p.y}`).join(" L ")} Z`}
            fill="none"
        ...
        /* Fallback to oval if GeoJSON not loaded */
